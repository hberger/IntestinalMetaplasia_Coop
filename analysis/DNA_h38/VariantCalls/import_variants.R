...
setwd("/data_genome2/projects/MB044_gastric_clones/analysis/DNA/VariantCalls/")

all_samples_folder = "/data_genome2/projects/MB044_gastric_clones/analysis/DNA/Strelka/called_variants/SnpEff"

all_vcf_raw = list()
# annotated variants, all samples
all_vcf_raw[["ESGIv2"]] = readVcf(file.path(all_samples_folder, "ESGI_IM_Strelka_calls_merged_SnpEff.vcf.gz"))

all_samples = list()
all_samples[["ESGIv2"]] = c(paste("4447_",c("D","E","F","H","I","J","K","L"), sep=""))
...
all_anno = list()
all_anno[["ESGIv2"]] = fread("./Variant_annotation_ESGIv2.txt", sep="\t", header=T, stringsAsFactors = F)
...
save(all_variants, all_samples, anno_unique_sites, anno_unique_sites_highest_impact, file="ESGI_v2_Exome_Strelka_Calls.Rdata")
